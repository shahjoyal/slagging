...
function calculateAFT(values) {
  try {
    console.log(model.predict([values])[0]);
...
app.post("/calculate-aft", async (req, res) => {
 try {
    console.log(model.predict([req.body.values])[0]);
    return res.status(200).json({ prediction: model.predict([req.body.values])[0] });
  } catch (e) {
    console.warn("ML prediction failed, falling back to formula.");
    
    const [SiO2, Al2O3, Fe2O3, CaO, MgO, Na2O, K2O, SO3, Ti2O] = req.body.values;
    const sumSiAl = SiO2 + Al2O3;

    if (sumSiAl < 55) {
      return res.status(200).json({ prediction: 1245 + 1.1*SiO2 + 0.95*Al2O3 - 2.5*Fe2O3 - 2.98*CaO - 4.5*MgO -
             7.89*(Na2O+K2O) - 1.7*SO3 - 0.63*Ti2O });
    } 
    else if (sumSiAl < 75) {
      return res.status(200).json({ prediction: 1323 + 1.45*SiO2 + 0.683*Al2O3 - 2.39*Fe2O3 - 3.1*CaO - 4.5*MgO -
             7.49*(Na2O+K2O) - 2.1*SO3 - 0.63*Ti2O });
    } 
    else {
      return res.status(200).json({ prediction: 1395 + 1.2*SiO2 + 0.9*Al2O3 - 2.5*Fe2O3 - 3.1*CaO - 4.5*MgO -
             7.2*(Na2O+K2O) - 1.7*SO3 - 0.63*Ti2O });
    }
  }
})
